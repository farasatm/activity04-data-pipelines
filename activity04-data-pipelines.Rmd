...
**With message turned on ("TRUE"), the document shows the log of what was loaded and if there were any conflicts**:
...
**To find the major with lowest unemployment rate, I will sort the dataset using the column unemployment_rate, and then select the first one given that its sorted in ascending order, Same thing I will do to find highest percentage of women, I will sort the dataset based on sharewomen column and sort it by descending order, for the third one, I will serparate the stem major by filtering based on the major_code and comparing them against median **:
...
**Zoology, Ecology, Communication Technologies, Neuroscience, and Environmental Science majors all have median salaries below $36,000, meaning they make less than the median salary for all majors.**:
